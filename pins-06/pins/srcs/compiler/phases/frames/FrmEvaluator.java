...
public class FrmEvaluator extends AbsFullVisitor<Object, FrmEvaluator.Context> {
	...
	@Override
	public Object visit(AbsFunDef def, FrmEvaluator.Context visArg){
		FrmEvaluator.FunContext context = new FrmEvaluator.FunContext();
		context.depth = ((FunContext) visArg).depth + 1;
		//context.argsSize +=
		super.visit(def, context);
		Label label;
		label = context.depth == 1 ? new Label(def.name) : new Label();
		Frames.frames.put(def, new Frame(label, context.depth, context.locsSize, context.argsSize));
		return null;
	}
