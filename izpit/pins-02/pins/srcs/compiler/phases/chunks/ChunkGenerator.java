	...
	private Vector<ImcStmt> linearize(Vector<ImcStmt> stmts) {
		...
		for (int s = 0; s < stmts.size(); s++) {
			...
			if (stmt instanceof ImcCJUMP) {
				...
				//((ImcCJUMP)stmt).cond.accept(this, null);
				//Long cond = ImcGen.exprImCode.get();
				//Label l = (cond != 0) ? ((ImcCJUMP)stmt).posLabel : ((ImcCJUMP)stmt).negLabel;
				linearStmts.add(new ImcJUMP(((ImcCJUMP)stmt).posLabel));
				linearStmts.add(new ImcLABEL(negLabel));
				linearStmts.add(new ImcJUMP(((ImcCJUMP)stmt).negLabel));
			}
			else
